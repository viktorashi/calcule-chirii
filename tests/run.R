...
for (rates in list(5, c(5, 6, 7, 8), c(8, 7, 6, 5), c(5, 7, 4, 6, 3))) {
  for (cap in c(Inf, 5.5, 4.8)) {
    for (n in 1:12) {
      ...
      check(t1$is_forecast, r$is_forecast)
      check(t1$rate_type, r$rate_type)
      check(t1$payment_date, r$payment_date)
...
shiny::testServer(server, {
  ...
  session$setInputs(chirie_eur = 500, c2_n = 24, durata = 24)
